...
test("この素晴らしい世界に祝福を！2", async ({ page }, testInfo) => {
  const config = {
    seriesName: testInfo.title,
    selector: `a.thumb_anchor.g-video-link`,
    homePage: "https://ch.nicovideo.jp/konosuba2",
  }

  await autoDownloadDanmaku(page, config)
})
test("幼女戦記", async ({ page }, testInfo) => {
  const config = {
    seriesName: testInfo.title,
    selector: `a.thumb_anchor.g-video-link`,
    homePage: "https://ch.nicovideo.jp/youjo-senki",
  }
