...
test("ようこそ実力至上主義の教室へ", async ({ page }, testInfo) => {
  const config = {
    seriesName: testInfo.title,
    selector: `a.thumb_anchor.g-video-link`,
    homePage: "https://ch.nicovideo.jp/you-zitsu",
  }

  await autoDownloadDanmaku(page, config)
})
test("終物語2", async ({ page }, testInfo) => {
  const config = {
    seriesName: testInfo.title,
    selector: `a.thumb_anchor.g-video-link`,
    homePage: "https://ch.nicovideo.jp/owarimonogatari2",
  }
