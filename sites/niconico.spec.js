...
test("ダンジョンに出会いを求めるのは間違っているだろうか", async ({ page }, testInfo) => {
  const config = {
    seriesName: testInfo.title,
    selector: `a.thumb_anchor.g-video-link`,
    homePage: "https://ch.nicovideo.jp/danmachi",
  }

  await autoDownloadDanmaku(page, config)
})
test("グリザイアの楽園", async ({ page }, testInfo) => {
  const config = {
    seriesName: testInfo.title,
    selector: `a.thumb_anchor.g-video-link`,
    homePage: "https://ch.nicovideo.jp/grisaia_rakuen",
  }

  await autoDownloadDanmaku(page, config)
})
test("ニセコイ2", async ({ page }, testInfo) => {
  const config = {
    seriesName: testInfo.title,
    selector: `a.thumb_anchor.g-video-link`,
    homePage: "https://ch.nicovideo.jp/nisekoi2",
  }
